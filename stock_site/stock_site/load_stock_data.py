...
class Data():
    ...
    def load_data_from_csv(self, year_period=1):
        ...
        print("Loading daily investor")
        self.daily_investor_data = self.load_daily_data('daily_investor', year_period)
    ...
    def combine_data(self):
        ...
        self.data['ForeignInvestBuy'] = self.get_series_data(self.daily_investor_data, '外資買進股數') # 買賣超 Net Buy / Net Ssell
        self.data['ForeignInvestSell'] = self.get_series_data(self.daily_investor_data, '外資賣出股數') 
        self.data['ForeignInvestNBS'] = self.get_series_data(self.daily_investor_data, '外資買賣超股數') 
        self.data['InvestTrustBuy'] = self.get_series_data(self.daily_investor_data, '投信買進股數') 
        self.data['InvestTrustSell'] = self.get_series_data(self.daily_investor_data, '投信賣出股數') 
        self.data['InvestTrustNBS'] = self.get_series_data(self.daily_investor_data, '投信買賣超股數') 
        self.data['DealerBuy'] = self.get_series_data(self.daily_investor_data, '自營商買進股數(自行買賣)') 
        self.data['DealerSell'] = self.get_series_data(self.daily_investor_data, '自營商賣出股數(自行買賣)') 
        self.data['DealerNBS'] = self.get_series_data(self.daily_investor_data, '自營商買賣超股數') 
        self.data['ThreeInvestorNBS'] = self.get_series_data(self.daily_investor_data, '三大法人買賣超股數') 
